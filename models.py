...
class Weibo(db.Model, ModelHelper):
    ...
    def valid(self):
        return len(self.weibo) > 2 and len(self.weibo) < 31
...
class Comment(db.Model, ModelHelper):
    ...
    def valid(self):
        return len(self.comment) >= 3 and len(self.comment) <= 20
...
class User(db.Model, ModelHelper):
    ...
    def __init__(self, form):
        ...
        rand_url = '/static/img/{}.gif'.format(rand_num)
        self.avatar = form.get('avatar', rand_url)
